/* Ascen theme script: branding + render dynamic lists + age tabs + FAQ */
...
function setBranding() {
  ...
  document.documentElement.style.setProperty('--theme-accent', school.colors.accent ?? '#6c5ce7');
...
function ensureCardLiteStyle() {
  ...
  style.textContent = `
    ...
    .card-tag{display:inline-block;padding:6px 12px;border-radius:999px;background:rgba(108,92,231,.14);border:1px solid rgba(108,92,231,.22);font-weight:950;font-size:.78rem;margin-bottom:10px}
  `;
...
function initAgeTabs() {
  ...
  const panels = document.querySelectorAll('.age-panel');
...
function initFaqAccordion() {
  ...
  items.forEach((item) => {
    ...
    if (q && q.getAttribute('aria-expanded') === 'true') item.dataset.open = 'true';
  });
